...
function createPluginWithCachedHighlighter(
...
export default function shiki<Themes extends string = string>(
  ...
  // Create a single highlighter instance at the plugin level
  let highlighterPromise: Promise<Highlighter> | null = null;

  const getHighlighter = () => {
    if (!highlighterPromise) {
      const config = "themes" in options
        ? merge(multiThemeDefaults, options).highlighter
        : merge(singleThemeDefaults, options).highlighter;
  ...
  if ("themes" in options) {
    const mergedOptions = merge(multiThemeDefaults, options);
    return createPluginWithCachedHighlighter(mergedOptions, getHighlighter);
  }
  const mergedOptions = merge(singleThemeDefaults, options);
  return createPluginWithCachedHighlighter(mergedOptions, getHighlighter);
}
